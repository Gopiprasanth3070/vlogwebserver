...
class AuthController {
  ...
  public authGoogleCallback = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const code = req.query.code as string
      ...
      if (action === "signin") {
        ...
        if (user) {
          ...
          res.redirect(`${appConfig.authRedirectUrl}?action=signin&token=${token}`)
        } else {
  ...
  public getFacebookAuthURL = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const action = req.body.action
      const facebookAuthURL = this.facebookService.getFacebookAuthURL({ action })
      res.send({
  ...
  public authFacebookCallback = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const code = req.query.code as string
      const action = req.query.action as string
      console.log({ code, action })
      const facebookUser = await this.facebookService.getFacebookUser({ code, action })
      if (action === "signin") {
        const email = facebookUser.email
        const user = await this.usersService.findUser({ email })
        if (user) {
          const token = jwt.sign({ id: user.id }, "secret")
          return res.redirect(`${appConfig.authRedirectUrl}?action=signin&token=${token}`)
          // return res.send({
          //   object: "user",
          //   data: {
          //     ...user,
  ...
  public signin = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (user) {
        ...
        if (isMatch) {
          const token = jwt.sign({ id: user.id }, "secret")
          return res.send({ object: "user", data: { ...user, token }, message: "signin success" })
        }
      }
      return res.send({
        object: "error",
